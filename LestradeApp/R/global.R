# ============================================================================
# R/global.R — Fonctions utilitaires du package LestradeApp
# Les library() sont gérés par DESCRIPTION (Imports)
# DB_PATH est défini dynamiquement via Sys.getenv (setenv dans run.R)
# ============================================================================

`%||%` <- function(x, y) if (is.null(x) || length(x) == 0) y else x

# DB_PATH relit Sys.getenv à chaque appel → toujours synchronisé avec run()
if (exists("DB_PATH", envir = environment(), inherits = FALSE)) {
  rm("DB_PATH", envir = environment())
}
makeActiveBinding("DB_PATH",
  function() Sys.getenv("LESTRADE_DB_PATH", unset = "questionnaires.db"),
  environment()
)
...
# Génère le QR code — encode UNIQUEMENT les métadonnées + UID
# Le questionnaire complet est sur Drive, le mobile le télécharge via l'UID
export_quest_to_qr <- function(quest_id) {
  ...
  # Payload léger : juste ce qu'il faut pour retrouver le questionnaire sur Drive
  meta <- list(
    v   = "1.0",              # version Lestrade
    uid = uid,                # identifiant unique
    nom = full$questionnaire$nom,
  ...
  list(
    ...
    nom         = full$questionnaire$nom,
    n_questions = nrow(full$questions),
    n_chars     = nchar(json_str)   # Toujours < 300 chars → QR lisible garanti
  )
}
...
# init_db() est appelé depuis inst/app/global.R au démarrage de l'app Shiny
