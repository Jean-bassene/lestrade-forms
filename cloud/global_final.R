...
# Cloud : DB dans tempdir (persistante pour la session shinyapps)
IS_CLOUD <- TRUE
DB_PATH  <- file.path(tempdir(), "lestrade_cloud.db")
...
# Cloud : URL panier depuis variable d'environnement shinyapps.io
# Configurer dans shinyapps.io → App → Settings → Environment Variables
# Nom : LESTRADE_PANIER_URL  Valeur : https://script.google.com/macros/s/.../exec

get_panier_url <- function() {
  url <- Sys.getenv("LESTRADE_PANIER_URL", unset = "")
  if (nzchar(url)) url else NULL
}

save_panier_url <- function(url) {
  # Sur cloud, on ne peut pas sauvegarder — message informatif
  message("Cloud : URL panier définie via variable d'environnement LESTRADE_PANIER_URL")
}

# ── Création automatique du Sheet panier ─────────────────────────────────────
# Crée le Google Sheet + retourne son URL pour guider le déploiement Apps Script
# NB : le déploiement Apps Script nécessite une action manuelle (scope restreint Google)
create_panier_automatique <- function() {
  ...
  if (is.null(token)) stop("Connectez-vous d'abord à Google Drive.")

  # Créer le Google Sheet avec les deux feuilles attendues par le Apps Script
  googlesheets4::gs4_auth(token = token)
  ss <- googlesheets4::gs4_create(
    name   = "Lestrade_Panier",
    sheets = list(
      Panier = data.frame(
        quest_id = character(), uuid = character(),
        horodateur = character(), donnees_json = character(), recu_le = character()
      ),
      Questionnaires = data.frame(
        uid = character(), nom = character(),
        quest_json = character(), publie_le = character()
      )
    )
  )
  sheet_id  <- as.character(googledrive::as_id(ss))
  sheet_url <- paste0("https://docs.google.com/spreadsheets/d/", sheet_id)
  message("Sheet créé : ", sheet_url)

  list(ok = TRUE, sheet_id = sheet_id, sheet_url = sheet_url)
}
