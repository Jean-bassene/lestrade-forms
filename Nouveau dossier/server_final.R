...
server <- function(input, output, session) {
  ...
  # Utilise PowerShell pour éviter les problèmes d'encodage Windows FR
  .local_ip <- tryCatch({
    ps_cmd <- paste0(
    ...
    ip <- trimws(system(ps_cmd, intern = TRUE))
    if (length(ip) > 0 && grepl("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$", ip[1])) ip[1] else "127.0.0.1"
  ...
  setup_analytics <- function(bundle_fn, pfx) {
    ...
    output[[oid("analytics_gtsummary_message")]] <- renderUI({
      if (!requireNamespace("gtsummary",quietly=TRUE))
        return(div(class="hint","Package gtsummary non installé. Installez-le pour activer ce tableau."))
      NULL
    })
    output[[oid("analytics_gtsummary_table")]] <- renderDT({
      req(requireNamespace("gtsummary",quietly=TRUE))
      bundle  <- bundle_fn(); df <- bundle$data; meta <- bundle$meta
